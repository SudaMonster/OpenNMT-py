...
def build_aux_model(model):
    model_aux = onmt.Models.AuxiliaryNMTModel(
        model.encoder,
        model.decoder
    )
    return model_aux
...
def context(model, fields, optim, data_type, model_opt):
    train_loss = make_loss_compute(model, fields["tgt"].vocab, opt)
    valid_loss = make_loss_compute(model, fields["tgt"].vocab, opt,
                                   train=False)
    test_loss = make_loss_compute(model, fields["tgt"].vocab, opt,
                                   train=False)
    ...
    model_aux = build_aux_model(model)

    valid_aux_loss = make_loss_compute(model, fields["tgt"].vocab, opt,
                                   train=False)
    trainer_aux = onmt.Trainer(model_aux, train_loss, valid_aux_loss, optim,
                           trunc_size, shard_size, data_type,
                           norm_method, grad_accum_count)

    print('\nStart score...')

    valid_iter = make_dataset_iter(
        lazily_load_dataset("valid"),
        fields,
        opt,
        is_train=False
    )
    ...
    torch.save(
        hidden_states_aux,
        opt.data + '.aux.hidden.pt'
    )
    print('Auxiliary decoder perplexity on test: %g' % valid_stats_aux.ppl())
    valid_iter = make_dataset_iter(
        lazily_load_dataset("valid"),
        fields,
        opt,
    ...
    print('Main decoder')
    valid_stats, hidden_states = trainer.validate(valid_iter)
    torch.save(
        hidden_states,
        opt.data + '.main.hidden.pt'
    )
    print('Main decoder perplexity on test: %g' % valid_stats.ppl())
...
def main():
    ...
    # Do training.
    context(model, fields, optim, data_type, model_opt)
